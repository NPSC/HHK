...
(function ($) {
  $.fn.smsDialog = function (options) {
    var uiMkup = {
      smsTabs: `<div id="smsTabs" class="ui-tabs ui-corner-all, ui-widget ui-widget-content ui-tabs-vertical ui-helper-clearfix ui-corner-all">
                    <ul class="ui-tabs-nav ui-corner-all ui-helper-reset ui-helper-clearfix ui-widget-header"></ul>
            </div> `,
      msgsTabMkup: `<div id="msgsTabContent" class="hhk-overflow-x ui-tabs-panel ui-corner-all ui-widget-content">
      ...
      allGuestsTabMkup: `<div id="allGuestsTabContent" class="hhk-overflow-x ui-tabs-panel ui-corner-all ui-widget-content">
                <h4 class="msgTitle"></h4>
      ...
      campaignTabMkup: `<div id="campaignTabContent" class="hhk-overflow-x ui-tabs-panel ui-corner-all ui-widget-content">
                <h4 class="msgTitle"></h4>
      ...
      msgMkup: `<div class="msgContainer hhk-flex"> 
                    <div class="msg">
  ...
  function actions($dialog, uiMkup, settings, $btn) {
    ...
    $dialog.on("click", "#campaignTabContent .sendCampaign", function () {
      ...
      if (settings.guestCount == 0) {
        flagAlertMessage(
          "There are no guests to send message to. Change the filter option and try again.",
        );
        return false;
      }
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (settings.guestData.length > 1 && phoneCount > 0) {
            ...
            $dialog
              .find("#smsTabs ul")
              .append($(`<li><a href="#allGuestsTabContent">Current Guests</a></li>`));

            let disabledTabs = [];

            $.each(settings.guestData, function (i, guest) {
              let li = $(`<li><a href="#msgsTabContent">` + guest.Name_Full + `</a></li>`).data(
                guest,
              );
              if (guest.isMobile == 0) {
            ...
            $dialog
              .find("#smsTabs")
              .append(uiMkup.msgsTabMkup + uiMkup.allGuestsTabMkup)
              .tabs({
                active: 0,
                ...
                create: function (event, ui) {
                  if (ui.tab && ui.tab.data("Name_Full")) {
                    ui.panel
                      .find(".msgTitle")
                      .text(ui.tab.data("Name_Full") + " - " + ui.tab.data("Phone_Num"));
                    ui.panel
                      .find(".newMsg textarea")
                      .attr("placeholder", "Message " + ui.tab.data("Phone_Num") + "...")
                      .val("");
                    ui.panel
                      .find(".newMsg button[name=sendMsg]")
                      .data("idname", ui.tab.data("idName"));
                    var $msgsContainer = ui.panel
                      .find(".msgsContainer")
                      .empty()
                      .addClass("loading");
                  ...
                  $dialog
                    .find("#allGuestsTabContent .newMsg textarea")
                    .attr("placeholder", "Message...")
                    .val("");
                  $dialog
                    .find("#allGuestsTabContent .newMsg button[name=sendMsg]")
                    .data("idname", "");
                  $dialog.find("#allGuestsTabContent .allRecipients").empty().html("<h5>To:</h5>");
                  $.each(settings.guestData, function (i, guest) {
                    if (guest.isMobile) {
                      $dialog
                        .find("#allGuestsTabContent .allRecipients")
                        .append(
                          "<span title='" + guest.Phone_Num + "'>" + guest.Name_Full + "</span>",
                        );
                    }
                  });
                },
                beforeActivate: function (event, ui) {
                  if (ui.newTab && ui.newTab.data("Name_Full")) {
                    ui.newPanel
                      .find(".msgTitle")
                      .text(ui.newTab.data("Name_Full") + " - " + ui.newTab.data("Phone_Num"));
                    ui.newPanel
                      .find(".newMsg textarea")
                      .attr("placeholder", "Message " + ui.newTab.data("Phone_Num") + "...")
                      .val("");
                    ui.newPanel
                      .find(".newMsg button[name=sendMsg]")
                      .data("idname", ui.newTab.data("idName"));
                    var $msgsContainer = ui.newPanel
                      .find(".msgsContainer")
                      .empty()
                      .addClass("loading");
            ...
            $dialog
              .find(".msgTitle")
              .text(settings.guestData[0].Name_Full + " - " + settings.guestData[0].Phone_Num);
            $dialog
              .find(".newMsg textarea")
              .attr("placeholder", "Message " + settings.guestData[0].Phone_Num + "...")
              .val("");
            $dialog
              .find(".newMsg button[name=sendMsg]")
              .data("idname", settings.guestData[0].idName);
            var $msgsContainer = $dialog.find(".msgsContainer").empty().addClass("loading");
            loadMsgs($msgsContainer, uiMkup, settings, settings.guestData[0].idName);
            $dialog.removeClass("loading");
          } else if (settings.guestData.length == 0 || phoneCount === 0) {
            $dialog.html(
              "<div class='ui-state-error ui-corner-all p-2'>No guests have opted in to receive text messages.</div>",
            );
            $dialog.removeClass("loading");
          }
        },
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (settings.guestData.contacts.length > 0) {
            ...
            if (
              settings.guestData.filterOptions !== undefined &&
              typeof settings.guestData.filterOptions == "object"
            ) {
              ...
              filterSelector.on("change", function () {
                ...
                $.each(settings.guestData.contacts, function (i, contact) {
                  if (
                    filterSelector.val() == "" ||
                    contact[settings.guestData.filterBy.Code] == filterSelector.val()
                  ) {
                    guestStr +=
                      contact.Name_First +
                      " " +
                ...
                $dialog
                  ...
                  .empty()
                  .html(
                    "<h5>To:</h5><span class='hhk-tooltip' id='collapsedrecipients' title=''>" +
                      settings.guestData.title +
                      (filterSelector.val().length > 0
                        ? " - " + settings.guestData.filterOptions[filterSelector.val()].Description
                        : "") +
                      " (" +
                      settings.guestCount +
                      ")</span>",
            ...
            $.each(settings.guestData.contacts, function (i, contact) {
              guestStr +=
                contact.Name_First + " " + contact.Name_Last + " - " + contact.Phone_Num + "<br>";
              settings.guestCount++;
            });
            ...
            $dialog
              ...
              .html(
                "<h5>To:</h5><span class='hhk-tooltip' id='collapsedrecipients' title=''>" +
                  settings.guestData.title +
                  " (" +
                  settings.guestData.contacts.length +
                  ")</span>",
              );
            ...
            $dialog.html(
              "<div class='ui-state-error ui-corner-all p-2'>No guests have opted in to recieve text messages.</div>",
            );
            $dialog.removeClass("loading");
          }
        },
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.error) {
          ...
          var $msgsTab = $msgsContainer.parent();
          if (data.subscriptionStatus === false) {
            $msgsTab
              .find(".msgTitle")
              .append(
                "<span class='smsUnsubscribed ui-corner-all p-1 ui-state-error' title='The user has unsubscribed and will not receive any messages.'>Unsubscribed</span>",
              );
          ...
          $.each(data.msgs, function (i, msg) {
            ...
            if (msg.directionType == "MT") {
              $msgMkup
                .addClass("houseMsg")
                .find(".msgMeta")
                .text(data.siteName + " - " + timestamp.calendar());
            } else if (msg.directionType == "MO") {
              $msgMkup
                .addClass("guestMsg")
                .find(".msgMeta")
                .text(data.Name_First + " - " + timestamp.calendar());
            }

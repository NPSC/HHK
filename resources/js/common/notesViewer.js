...
(function ($) {
  ...
  $.fn.notesViewer = function (options) {
    ...
    if (
      settings.linkType == "staff" ||
      settings.linkType == "curguests" ||
      settings.linkType == "waitlist" ||
      settings.linkType == "confirmed" ||
      settings.linkType == "unconfirmed"
    ) {
      settings.dtCols.forEach(function (El, Index) {
        if (El.targets > 3) {
          ...
          settings.dtCols.push({
            ...
            render: function (data) {
              if (settings.staffNoteCats[data]) {
                return (
                  settings.staffNoteCats[data].Description + '<span data-cat="' + data + '"></span>'
                );
              }
              return data;
            },
  ...
  function createNewNote(settings, dtTable, $wrapper) {
    ...
    if (settings.linkId >= 0) {
      $button = $(
        '<button class=" ui-button ui-corner-all ui-widget mt-2 mt-md-0" id="note-newNote" style="min-width:fit-content">Save New Note</button>',
      ).click(function (e) {
        e.preventDefault();
  ...
  function createActions(noteId, row) {
    ...
    // Edit icon
    $li = $(
      '<li title="Edit Note" data-noteid="' + noteId + '" data-notetext="' + row.Note + '" />',
    ).addClass("hhk-note-button note-edit ui-corner-all ui-state-default");
    $li.append($('<span class="ui-icon ui-icon-pencil" />'));

    $ul.append($li);

    // Save(Done) Edit Icon
    $li = $('<li title="Save Note" />')
      .addClass("hhk-note-button note-done note-action ui-corner-all ui-state-default")
      .hide();
    ...
    // Cancel Edit Icon
    $li = $('<li title="Cancel" data-titletext="' + row.Title + '" />')
      .addClass("hhk-note-button note-cancel note-action ui-corner-all ui-state-default")
      .hide();
    ...
    // Delete Edit Icon
    $li = $('<li title="Delete Note" data-noteid="' + noteId + '" />').addClass(
      "hhk-note-button note-delete ui-corner-all ui-state-default",
    );
    ...
    $li = $('<li title="Undo Delete" data-noteid="' + noteId + '" />')
      .addClass("hhk-note-button note-undodelete ui-corner-all ui-state-default")
      .hide();
  ...
  function actions($wrapper, settings, $table) {
    ...
    $wrapper.on("click", ".note-edit", function (e) {
      ...
      $(this)
        .closest("tr")
        .find(".noteCategory")
        .html(categorySelector(settings, selectedCategory));
      $(this)
        .closest("tr")
        .find(".noteText")
        .html(
          '<div class="hhk-flex align-items-center"><textarea class="p-2 hhk-autosize ui-widget-content ui-corner-all" style="width: 100%; height: ' +
            $(this).closest("tr").find(".noteText").height() +
            'px;" id="editNoteText">' +
            noteText +
            '</textarea><button type="button" class="note-done ui-button ui-corner-all ms-2" style="min-width: fit-content">Save Note</button></div>',
        );
    ...
    $wrapper.on("click", ".btnCat", function (e) {
      e.preventDefault();
      var $btnWrapper = $(this).closest("#noteCatBtns");
      $btnWrapper.find("button").removeClass("catActive");
      $(this).addClass("catActive");
      var searchVal = $(this).data("id");
      if (searchVal != "") {
        console.log(searchVal);
        $table
          .column(".noteCategory")
          .search("^" + searchVal + "$", true)
          .draw();
      } else {
  ...
  function categorySelector(settings, selected = false) {
    let $catSelect = $(
      '<select name="noteCategory" id="noteCategory" class="mt-2 mt-md-0 me-3"><option disabled ' +
        (selected == false ? "selected" : "") +
        ">-- Select Category --</option><option></option></select>",
    );
      ...
      if (k == selected) {
        $catSelect.append(
          '<option value="' +
            k +
            '" selected>' +
            settings.staffNoteCats[k].Description +
            "</option>",
        );
      } else {
        $catSelect.append(
          '<option value="' + k + '">' + settings.staffNoteCats[k].Description + "</option>",
        );
      }
    }
    return $catSelect;
  ...
  function categoryFilter(settings) {
    let $filterWrapper = $(
      '<div class="d-flex mt-2"><div class="d-md-none d-flex" style="align-items:center"><span class="ui-icon ui-icon-triangle-1-w"></span></div><div id="noteCatBtns"></div><div class="d-md-none d-flex" style="align-items:center"><span class="ui-icon ui-icon-triangle-1-e"></span></div></div>',
    );
    let $noteCatBtns = $filterWrapper.find("#noteCatBtns");
    $noteCatBtns.append('<button class="btnCat catActive" data-id="0">All</button>');
    for (var k in settings.staffNoteCats) {
      $noteCatBtns.append(
        '<button class="btnCat" data-id="' +
          k +
          '">' +
          settings.staffNoteCats[k].Description +
          "</button>",

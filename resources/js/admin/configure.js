...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if ($(".hhk-delMerchant").length > 0) {
    $(".hhk-delMerchant").on("change", function () {
      if (
        $(this).prop("checked") &&
        !confirm("Delete the " + $(this).data("merchant") + " merchant?")
      ) {
  ...
  if ($(".hhk-setMerchantRooms").length > 0) {
    $(".hhk-setMerchantRooms").on("change", function () {
      if (
        $(this).prop("checked") &&
        !confirm("This sets ALL the rooms to the " + $(this).data("merchant") + " merchant!")
      ) {
        $(this).prop("checked", false);
      }
    });
  ...
  var dtCronCols = [
        ...
        $.each(data, function (key, value) {
          if (value.length > 0 || (typeof value === "object" && value !== null)) {
            if (key == "fieldSet" && cronLookups.inputSets[value] !== undefined) {
              ...
              key == "filterOpts" &&
              cronLookups.filterOpts[data.report].data !== undefined
            ) {
              let filterOptValue = "";
              $.each(value, (k, v) => {
          ...
          mkup +=
            '<div class="hhk-flex"><span class="me-2"><strong>' +
    ...
    },
    {
      ...
      render: function (data, type, row) {
        return (
          '<div class="cronActions hhk-flex">' +
          ($("#canEditCron").val() == true
            ? '<button type="button" class="editCron ui-button ui-corner-all" data-job="' +
              data +
              '" data-jobtitle="' +
              row.Title +
              '" data-type="' +
              ...
              '">Edit</button>'
            : "") +
          '<button type="button" class="runCron ui-button ui-corner-all" data-job="' +
          data +
          '" data-dryRun="1">Dry Run</button>' +
          ($("#canForceRunCron").val() == true
            ? '<button type="button" class="runCron ui-button ui-corner-all" data-job="' +
              data +
              '" data-dryRun="0">Run Now</button>'
            : "") +
          '<button type="button" class="saveCron ui-button ui-corner-all" data-job="' +
          data +
          '" style="display:none;">Save</button>' +
          '<button type="button" class="deleteCron ui-button ui-corner-all" data-job="' +
          data +
          '" style="display:none;">Delete</button>' +
          '<button type="button" class="cancelCron ui-button ui-corner-all" data-job="' +
          data +
          '" data-jobtitle="' +
          row.Title +
          '" data-interval="' +
  ...
  function cronActions($wrapper, cronTable) {
    ...
    $wrapper.on("click", ".editCron", function (e) {
      ...
      var jobTitleMkup =
        '<input type="text" id="editJobTitle" style="width: 100%" value="' +
        $editBtn.data("jobtitle") +
        '">';
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            if (cronLookups.filterOpts[report]) {
              $row
                .find(".editParam[data-name=filterOpts]")
                .html(cronLookups.filterOpts[report].mkup);
            } else {
      ...
      render: function (data, _type) {
        ...
        $.each(json, function (k, v) {
          if (typeof v == "string") {
            ...
            detailStr +=
              "<div><strong>" + k + ":</strong><pre>" + JSON.stringify(v, null, 2) + "</pre></div>";
          }
        });
        return detailStr;
      },
  ...
  $("#financialRoomSubsidyId, #financialReturnPayorId").change(function () {
    ...
    if (
      $("#financialRoomSubsidyId").val() != 0 &&
      $("#financialRoomSubsidyId").val() === $("#financialReturnPayorId").val()
    ) {

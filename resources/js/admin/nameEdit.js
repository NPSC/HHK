...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function filterDefaultPageOptions() {
    ...
    $sel.find('option[value!=""]').each(function () {
      ...
      var visible =
        ...
        checkedCodes.some(function (c) {
          return groups.indexOf(c) >= 0;
        });
  ...
  createAutoComplete(
    $("#txtsearch"),
    3,
    { cmd: "srrel", basis: $("#rbmemEmail").prop("checked") ? "e" : "m", id: 0 },
    function (item) {
      if (item.id === "i") {
        // New Individual
  ...
  $(".toggle-docs-detail").toggle(
    function () {
      var theUl = $(this).text("Show details").parent().next("ul");
      theUl.find("li > div:first-child").removeClass("header-open").nextAll().hide();

...
(function ($) {
    ...
    var defaults = {
      ...
      dtCols: [
        ...
        },
        {
          ...
          render: function (data, type, row) {
            return (
              '<a href="#" class="formDetails" data-docid="' +
              row.idDocument +
              '" data-status="' +
              row.idStatus +
              '" data-resvid="' +
              row.idResv +
              '" data-enablereservation="' +
              row.enableReservation +
              '">' +
              data +
              "</a>"
            );
  ...
  function createMarkup($wrapper, settings) {
    ...
    //build status tabs
    $wrapper
      .find("#referralTabs ul")
      .append(
        '<li data-status="inbox"><a href="#referralTabContent">' +
          (settings.statuses["n"].icon
            ? '<span class="' + settings.statuses["n"].icon + '"></span>'
            : '<span class="ui-icon ui-icon-blank"></span>') +
          ' Inbox (<span class="referralCount">' +
          settings.statuses["n"].count +
          "</span>)</a></li>",
      );
      ...
      if (value.idStatus != "n" && value.idStatus != "ip") {
        $wrapper
          ...
          .append(
            '<li data-status="' +
              value.idStatus +
              '"><a href="#referralTabContent">' +
              (value.icon
                ? '<span class="' + value.icon + '"></span>'
                : '<span class="ui-icon ui-icon-blank"></span>') +
              " " +
              value.Status +
              ' (<span class="referralCount">' +
              settings.statuses[value.idStatus].count +
              "</span>)</a></li>",
          );
      }
    });
    ...
    return (
      `
			<ul class="gmenu" style="font-weight:normal">
				<li>Action
					<ul>
						<li class="formDetails" data-docid="` +
      idDocument +
      `" data-status="` +
      row.idStatus +
      `" data-resvid="` +
      ...
      `" data-enablereservation="` +
      row.enableReservation +
      `" title="Form Details"><div>View ` +
      (settings.labels.referralFormTitle || "Referral Form") +
      `</div></li>` +
      (row.idResv
        ? `<li class="formResv"><div><a href="Reserve.php?rid=` +
          row.idResv +
          `" style="text-decoration:none;">View ` +
          (settings.labels.reservation || "Reservation") +
          `</a></div></li>`
        : ``) +
      `<li></li>` +
      (row.idStatus == "ip"
        ? `<li class="formUpdateStatus" data-docid="` +
          idDocument +
          `" data-target="n" title="Mark as Unread"><div>Mark as Unread</div></li>`
        : "") +
      (row.idStatus != "n" && row.idStatus != "ip" && !row.idResv
        ? `<li class="formUpdateStatus" data-docid="` +
          idDocument +
          `" data-target="ip" title="Move to Inbox"><div>Move to Inbox</div></li>`
        : "") +
      (row.idStatus != "ar" && !row.idResv
        ? `<li class="formUpdateStatus" data-docid="` +
          idDocument +
          `" data-target="ar" title="Archive Form"><div>Archive</div></li>`
        : "") +
      (!row.idResv
        ? `<li class="formUpdateStatus" data-docid="` +
          idDocument +
          `" data-target="d" title="Delete Form"><div>Delete</div></li>`
        : "") +
      `</ul>
				</li>
			</ul>
  ...
  function actions($wrapper, settings, formDetailsDialog) {
    $wrapper.on("click", ".formDetails", function (e) {
      ...
      var dialogTitle =
        " for " +
        rowdata["Patient First Name"] +
        " " +
        rowdata["Patient Last Name"] +
        (rowdata["Expected Checkin"] != ""
          ? " expected " + dateRender(rowdata["Expected Checkin"], "display")
          : "");

      window.frames["formPreviewIframe"].resizeTo(1920, 1080);
      formDetailsDialog
        .find("#formDetailsIframe")
        .attr("src", settings.detailURL + "?form=" + idDocument);
  ...
  function reloadTotals($wrapper, settings) {
    $.ajax({
      ...
      success: function (data) {
        if (data.totals) {
          $wrapper
            .find("#referralTabs ul li[data-status=inbox] span.referralCount")
            .text(data.totals["n"].count);
          $("#spnNumReferral").text(data.totals["n"].count);
          $.each(data.totals, function (key, value) {
            $wrapper
              .find("#referralTabs ul li[data-status=" + key + "] span.referralCount")
              .text(value.count);
          });

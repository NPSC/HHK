  ...
  constructor(initData, options) {
    ...
    function FamilySection($wrapper) {
      ...
      function verifyAddress(prefix) {
        ...
        if (
          $("#" + prefix + "incomplete").length > 0 &&
          ($("#" + prefix + "incomplete").prop("checked") === false ||
            (isCheckin == true &&
              insistCkinAddress == true &&
              $("#" + prefix + "cbStay").prop("checked") === true))
        ) {
          // Look at each entry
          $("." + prefix + "hhk-addr-val")
            .not(".hhk-MissingOk")
          ...
          if (addrValid == false) {
            ...
            if (
              $("#" + prefix + "toggleAddr")
                .find("span")
                .hasClass("ui-icon-circle-triangle-s")
            ) {
        ...
        var isNoPhone =
          $('.prefPhone[id^="' + prefix + 'phno"]:checked').length === 0 ? false : true;

        if (
          isCheckin == true &&
          insistCkinPhone == true &&
          phoneFilled == false &&
          isNoPhone == false &&
          $("#" + prefix + "cbStay").prop("checked") === true
        ) {
        ...
        var isNoEmail =
          $('.prefEmail[id^="' + prefix + 'emno"]:checked').length === 0 ? false : true;
      ...
      function verifyEmergencyContacts(prefix) {
        ...
        // Emergency Contact
        if (
          $("#" + prefix + "cbEmrgLater").length > 0 &&
          $("#" + prefix + "cbEmrgLater").prop("checked") === false
        ) {
      ...
      function addrCopyDown(sourcePrefix) {
        for (var prefix in addrs.list()) {
          ...
          if (
            ...
            addrs.list()[sourcePrefix].Country_Code
          ) {
      ...
      function copyAddrSelector($button, prefix) {
        ...
        if (opts > 0) {
          ...
          var $selDiv = $(
            '<div id="divSelAddr" style="position:absolute; vertical-align:top;" class="hhk-addrPicker hhk-addrPickerPanel"/>',
          )
      ...
      function setAddrFlag($addrFlag) {
        ...
        if ($("#" + prefix + "incomplete").prop("checked") === true) {
          $addrFlag
            ...
            .attr("title", "Incomplete Address is checked");
          ...
          if (!isAddressComplete(prefix)) {
            $addrFlag
              ...
              .attr("title", "Address is Incomplete");
      ...
      function copyEmergContactSelector($button, prefix) {
        ...
        for (var p in emergContacts.list()) {
          if (emergContacts.list()[p].First != "" || emergContacts.list()[p].Last != "") {
            var notFound = true,
              optText =
                emergContacts.list()[p].First +
                " " +
                emergContacts.list()[p].Last +
                " " +
      ...
      function setEmergContact(prefix, p) {
        ...
        $("#" + prefix + "selEmrgRel")
          .val(emergContacts.list()[p].relation)
          .change();
      ...
      function initFamilyTable(data) {
        ...
        fDiv = $("<div/>")
          .addClass("ui-widget-content ui-corner-bottom hhk-tdbox hhk-overflow-x")
          .prop("id", divFamDetailId)
          .css("padding", "5px");
        ...
        expanderButton = $(
          "<ul style='list-style-type:none; float:right;margin-left:5px;padding-top:2px;' class='ui-widget'/>",
        ).append(
          $("<li class='ui-widget-header ui-corner-all' title='Open - Close'>").append(
            $("<span id='f_drpDown' class='ui-icon ui-icon-circle-triangle-n'></span>"),
          ),
        );
        ...
        $wrapper.empty().append(fHdr).append(fDiv).show();
      }
      ...
      function initIncidentSection() {
        if (idPsg) {
          ...
          iSection = $(
            '<div style="font-size: 0.9em; max-width: 100%; margin-bottom: 0.5em; margin-top: 0.5em; display: none;"/>',
          )
            .addClass("ui-widget hhk-visitdialog hhk-row")
            .prop("id", "incidentsSection");

          iHdr = $('<div style="padding:2px; cursor:pointer;"/>').addClass(
            "ui-widget-header ui-state-default ui-corner-all hhk-incidentHdr",
          );
          iDiv = $('<div style="padding: 5px;"/>')
            .addClass("ui-corner-bottom hhk-tdbox ui-widget-content")
            .prop("id", "incidentContent")
            .hide();

          //add header title, count span and open/close arrow
          iHdr
            .append(
              '<div class="hhk-checkinHdr" style="display: inline-block">Incidents<span id="incidentCounts"/></div>',
            )
            .append(
      ...
      function setUp(data) {
        ...
        if (
          data.famSection === undefined ||
          data.famSection.tblId === undefined ||
          data.famSection.tblId === ""
        ) {
          return;
        }
        ...
        $("input.hhk-zipsearch").each(function () {
          createZipAutoComplete($(this), "ws_admin.php", undefined, loadAddress);
        });
        ...
        if (setupComplete === false) {
          ...
          $("#" + divFamDetailId).on("click", ".hhk-removeBtn", function () {
            // Is the name entered?
            if (
              $("#" + $(this).data("prefix") + "txtFirstName").val() !== "" ||
              $("#" + $(this).data("prefix") + "txtLastName").val() !== ""
            ) {
              if (
                confirm(
                  "Remove this person: " +
          ...
          createAutoComplete(
            $("#txtemSch"),
            3,
            { cmd: "filter", add: "phone", basis: "psg", psg: data.idPsg },
            getECRel,
          );
      ...
      function newGuestMarkup(data, prefix) {
        ...
        $famTbl
          .find("tbody:first")
          .append($(data.ntr).addClass(stripeClass))
          .append($(data.atr).addClass(stripeClass));
      ...
      t.verify = function () {
        ...
        if ($deptDate.val() === "") {
          ...
          if (t.ciDate > t.coDate) {
            $arrDate.addClass("ui-state-error");
            msg +=
              "This " + resvTitle + "'s check-in date is after the expected departure date.<br>";
            isValid = false;
          }
        }
    ...
    function doOnDatesChange(dates) {
      ...
      if (hasIds) {
        ...
        var parms = {
          ...
          span: span,
          dt1:
            dates["date1"].getFullYear() +
            "-" +
            (dates["date1"].getMonth() + 1) +
            "-" +
            dates["date1"].getDate(),
          dt2:
            dates["date2"].getFullYear() +
            "-" +
            (dates["date2"].getMonth() + 1) +
            "-" +
            dates["date2"].getDate(),
          mems: people.list(),
        };
    ...
    function HospitalSection($hospSection) {
      ...
      t.setUp = function (hosp) {
        ...
        var expanderButton = $(
          ...
          $("<li class='ui-widget-header ui-corner-all' title='Open - Close'>").append(
            $("<span id='h_drpDown' class='ui-icon ui-icon-circle-triangle-n'></span>"),
          ),
        );
      ...
      t.verify = function () {
        ...
        if (t.setupComplete === true) {
          const $invalidInputs = $hospSection
            .find("select:required, input:required")
            .filter((i, el) => el.value == ""); //get required but empty inputs
    ...
    function ResvSection($wrapper) {
      ...
      function setupGatewayChooser(idx) {
        ...
        if (notThere) {
          $("#selccgw" + idx).append(
            '<option value="' + room.merchant + '">' + room.merchant + "</option>",
          );
        }
      ...
      function setUp(data) {
        $rDiv = $("<div/>")
          .addClass(
            " hhk-tdbox ui-corner-bottom hhk-tdbox ui-widget-content hhk-flex hhk-flex-wrap hhk-overflow-x",
          )
          .prop("id", "divResvDetail")
          .css({ padding: "5px" });
        ...
        if (data.resv.rdiv.rstat !== undefined) {
          ...
          $rDiv.on("change", ".hhk-checkboxlist", function () {
            if ($(this).prop("checked")) {
              ...
              $("#date" + $(this).data("code")).val(
                $("#date" + $(this).data("code")).prop("defaultValue"),
              );
        ...
        $expanderButton = $(
          ...
          $("<li class='ui-widget-header ui-corner-all' title='Open - Close'>").append(
            $("<span id='r_drpDown' class='ui-icon ui-icon-circle-triangle-n'></span>"),
          ),
        );
        ...
        if ($(".hhk-viewResvActivity").length > 0) {
          $(".hhk-viewResvActivity").click(function () {
            $.post(
              "ws_ckin.php",
              { cmd: "viewActivity", rid: $(this).data("rid") },
              function (data) {
                data = JSON.parse(data);
            ...
            $.post(
              ...
              function (d) {
                ...
                if (data.confrv) {
                  ...
                  var emailMkup =
                    '<div class="col-md-6 my-2 hhk-flex"><label for="confEmail" class="pe-2" style="min-width:fit-content;">To Address</label><input type="text" style="width:100%" id="confEmail" value="' +
                    data.email +
                    '"></div>';
                  emailMkup +=
                    '<div class="col-md-6 my-2 hhk-flex"><label for="ccConfEmail" class="pe-2" style="min-width:fit-content;">CC Address</label><input type="text" style="width:100%" id="ccConfEmail" value="' +
                    data.ccemail +
                    '"></div>';

                  $("#frmConfirm").append(
                    $(
                      '<div class="ui-dialog-buttonpane ui-widget-content ui-helper-clearfix hhk-flex hhk-flex-wrap">' +
                        emailMkup +
                        "</div>",
                    ),
                  );
      ...
      function validateCar(cnum) {
        ...
        var err = "";
        if (
          $("#car" + cnum + "txtVehLic").val() === "" &&
          $("#car" + cnum + "txtVehMake").val() === ""
        ) {
      ...
      function verify() {
        ...
        if (isCheckin) {
          if (t.checkPayments === true) {
            // Room rate
            if (
              $("#selCategory").val() == fixedRate &&
              $("#txtFixedRate").length > 0 &&
              $("#txtFixedRate").val() == ""
            ) {
            ...
            // Room fees paid
            if (
              $("input#feesPayment").length > 0 &&
              $("input#feesPayment").val() == "" &&
              insistPayFilledIn
            ) {
          ...
          // Return Pre-Payment?
          if (prePaymtAmt > 0 && window.isCheckedOut && $("#selexcpay").val() == "") {
          ...
          if (
            $("#selccgw").length > 0 &&
            $("#spnPayAmount").html() === "" &&
            ($("input[name=rbUseCard]:checked").val() == 0 ||
              $("input[name=rbUseCard]").prop("checked") === true)
          ) {
    ...
    function resvPicker(data, $resvDiag, $psgDiag) {
      ...
      $resvDiag.empty().append($(data.resvChooser)).children().find("input:button").button();
      ...
      $dialog
        ...
        .dialog("option", "buttons", {
          Open: function () {
            $(this).dialog("close");
            getReserve({
              idPsg: $dialog.find("input[name=cbselpsg]:checked").val(),
              id: data.id,
              cmd: "getResv",
            });
          },
        ...
        .dialog(
          ...
          data.patLabel +
            " Chooser" +
            (data.fullName === undefined ? "" : " For: " + data.fullName),
        )
        .dialog("open");
    }
    ...
    function loadResv(data) {
      ...
      if (data.deleted) {
        ...
        if (data.childDeleted && data.childDeleted > 0) {
          $("#contentDiv").append(
            "<p>Also deleted " + data.childDeleted + " child reservations.</p>",
          );
        }
      ...
      if (data.resv !== undefined) {
        ...
        if (data.rid > 0) {
          $("#btnDelete")
            .val("Delete " + resvTitle)
            .show();
          $("#btnShowReg").show();
          $("#spnStatus").text(
            data.resv.rdiv.rStatTitle === "" ? "" : " - " + data.resv.rdiv.rStatTitle,
          );
        }
      ...
      if (data.addPerson !== undefined) {
        if (people.addItem(data.addPerson.mem)) {
          ...
          if ($("#" + data.addPerson.mem.pref + "txtLastName").val() === "") {
            $("#" + data.addPerson.mem.pref + "txtLastName")
              .val(getSearchTerms("last"))
              .focus();
          }
          if ($("#" + data.addPerson.mem.pref + "txtFirstName").val() === "") {
            $("#" + data.addPerson.mem.pref + "txtFirstName")
              .val(getSearchTerms("first"))
              .focus();
          }
        }
      }
...
window.resvManager = resvManager; // used by reserve.js and checkingIn.js, loaded alongside this bundle

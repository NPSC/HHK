...
export function setupRates(ckIn) {
  ...
  $("#seladjAmount").change(function () {
    if ($selRateCat.val() !== fixedRate) {
      ...
      daysCalculator(
        ...
        0,
        adj,
        parseInt($("#spnNumGuests").text()),
        ckIn.idResv === undefined ? 0 : ckIn.idResv,
        function (rawAmt) {
  ...
  $selRateCat.change(function () {
    ...
    if (ckIn.resources[$selResource.val()].key > 0) {
      ...
      $("#keyDepAmt").val("");
      $(".hhk-kdrow").hide();
      $("#keyDepRx").prop("checked", false).change();
    }
  });
  ...
  $("#regDialog").dialog({
    ...
    buttons: {
      ...
      Save: function () {
        ...
        $(this).dialog("close");
        $.post("ws_ckin.php", { cmd: "saveReg", reg: idReg, parm: parms }, function (data) {
          if (!data) {
            alert("Bad Reply from Server");
            return;
          }

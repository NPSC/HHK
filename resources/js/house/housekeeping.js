...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $(
    "#btnReset1, #btnSubmitClean, #btnReset2, #btnPrintAll, #btnExcelAll, #btnSubmitTable, #prtCkOut, #prtCkIn, #prtClnToday",
  ).button();
    ...
    beforeActivate: function (event, ui) {
      if (ui.newPanel.length > 0) {
        if (ui.newTab.prop("id") === "lishoCL" && !listEvtTable) {
          ...
          ui.newPanel.find("table#dirtyTable.dataTable").length > 0
        ) {
    ...
    onClose: function (dateText) {
      ...
      if (d != coDate) {
        ...
        $("#inTable")
          .DataTable()
          .ajax.url(
            "ws_resc.php?cmd=cleanStat&tbl=inTable&stdte=" +
        ...
        var updatedBtn = getDtBtns(
          "Housekeeping - " +
            labels.visitor +
            "s Checking In - " +
            $.datepicker.formatDate("M d, yy", coDate),
        )[0];
  ...
  $("#inButtonSet.week-button-group").on("click", "button", function (_e) {
    ...
    if (btn.data("weeks")) {
      ...
      $("#inTable")
        ...
        .ajax.url(
          "ws_resc.php?cmd=cleanStat&tbl=inTable&stdte=" +
            $.datepicker.formatDate("yy-mm-dd", startDate) +
            "&enddte=" +
      ...
      var updatedBtn = getDtBtns(
        "Housekeeping - " +
          labels.visitor +
          "s Checking In - " +
          $.datepicker.formatDate("M d, yy", startDate) +
          " to " +
          $.datepicker.formatDate("M d, yy", endDate),
      )[0];
      $("#inTable").DataTable().button(0).remove().add(0, updatedBtn);
      $("#inTable").DataTable().ajax.reload();
    ...
    onClose: function (dateText) {
      ...
      if (d != coDate) {
        ...
        $("#outTable")
          .DataTable()
          .ajax.url(
            "ws_resc.php?cmd=cleanStat&tbl=outTable&stdte=" +
              $.datepicker.formatDate("yy-mm-dd", coDate) +
              "&enddte=" +
              $.datepicker.formatDate("yy-mm-dd", coDate),
          );

...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $.ajax({
    ...
    success: function (ajaxData) {
      ...
      let formData, formSuccessTitle, formSuccessContent;
      ...
      if (ajaxData.formData && ajaxData.formSettings) {
          ...
          layoutTemplates: {
            noLabel: function (field, label, help, data) {
              if (data.type == "paragraph") {
                ...
                return $("<div/>")
                  .addClass(data.width + " field-container")
                  .append(field);
              } else if (data.type == "button") {
                return $("<div/>")
                  .addClass(data.width + " mb-3 field-container")
                  .append(field);
              } else {
                return $("<div/>")
                  .addClass(data.width + " field-container")
            ...
            default: function (field, label, help, data) {
                ...
                return $("<div/>")
                  ...
                  .append(
                    $("<div/>")
                      .addClass("card")
                      .append(
                        $("<div/>").addClass("card-body").append(label, field, help, validation),
                      ),
                  );
              ...
              return $("<div/>")
                .addClass(data.width + " mb-3 field-container")
                .append(
                  $("<div/>").addClass("form-floating").append(field, label, help, validation),
                );
            },
          },
          i18n: {
        ...
        //zip code search
        $renderedForm.find("input.hhk-zipsearch").each(function () {
          createZipAutoComplete($(this), "ws_forms.php", undefined, null);
        });
        ...
        function addGuest() {
          ...
          $.each(guestGroup, function (key, element) {
            ...
            if (newElement.name) {
              newElement.name = newElement.name.replace(/\.g([0-9]+)\./gi, ".g" + guestIndex + ".");
              newElement.name = newElement.name.replace(
                /([a-z,-]+-[0-9]*-)([0-9]{1,2})$/gi,
                "$1" + guestIndex,
              );
            }
        ...
        function removeGuest(guestIndex) {
          ...
          userData = userData.filter(
            (element) => !(element.group == "guest" && element.guestIndex == guestIndex),
          );
          ...
          $.ajax({
            ...
            success: function (data, _textStatus, _jqXHR) {
                ...
                $.each(data.errors, function (key, error) {
                  if (key == "server") {
                    ...
                    $('form *[name="' + error.field + '[]"]')
                      ...
                      .addClass("is-invalid");
                    $('.validationText[data-field="' + error.field + '"')
                      .addClass("invalid-feedback")
                      .html(error.error);
                    $(".errmsg .alert-heading").text("Error");
                    $(".errmsg #errorcontent").text(
                      "You have validation errors in your submission, please correct the fields marked in red and try again.",
                    );
                    $(".errmsg").show();
                  }
                });
              ...
              if (data.status == "success") {
                ...
                $(".rendered-form input, .rendered-form textarea, .rendered-form select").attr(
                  "disabled",
                  "disabled",
                );
            ...
            error: function (_data, _textStatus, _errorThrown) {
            ...
            statusCode: {
              501: function () {
                $(".errmsg .alert-heading").text("Error");
                $(".errmsg #errorcontent").text(
                  "You have invalid special characters in your submission. Is your cat on your keyboard? Please check your form and try again.",
                );
                $(".errmsg").show();
              },
              500: function () {
                ...
                $(".errmsg #errorcontent").text(
                  "We are unable to process your submission at this time due to a server error. We're hard at work fixing the issue. Please try again in a few minutes.",
                );
                $(".errmsg").show();
              },
              403: function () {

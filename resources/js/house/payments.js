...
function doHouseWaive(p, a) {
  if (a.overPayAmt < 0 || (a.overPayAmt == 0 && a.totCharges > 0)) {
    ...
    if (p.houseWaiveCb.prop("checked")) {
      ...
      const updateFeeTaxes = new UpdateTaxes(p.taxingItems);
      let houseWaive =
        Math.max(a.roomBalDue - a.feePayPreTax, 0) + a.invAmt + a.vfee - a.totReturns;
    ...
    $("#txtInvSearch" + index).keypress(function (event) {
      ...
      if (event.keyCode == "13") {
        if (mm == "" || !isNumber(parseInt(mm, 10))) {
          ...
          $.getJSON(
            "../house/roleSearch.php",
            { cmd: "filter", basis: "ba", letters: mm },
            function (data) {
              try {
                data = data[0];
              } catch (err) {
...
export function daysCalculator(
  ...
  if (
    ...
    (days != gblRtCalcParams.days ||
      idRate != gblRtCalcParams.idRate ||
      idVisit != gblRtCalcParams.idVisit ||
      fixedAmt != gblRtCalcParams.fixedAmt ||
      adjAmt != gblRtCalcParams.adjAmt ||
      numGuests != gblRtCalcParams.numGuests ||
      idResv != gblRtCalcParams.idResv)
  ) {
    var parms = {
      cmd: "rtcalc",
      vid: idVisit,
...
export function showReceipt(dialogId, markup, title, width, paymentId = false, billToEmail = "") {
  ...
  const downloadBtn = $(
    "<a href='ws_ckin.php?cmd=downloadReceipt&paymentId=" +
      paymentId +
      "' class='ms-3'><i class='bi bi-cloud-arrow-down-fill me-3'></i>Download</a>",
  );
...
export function paymentRedirect(data, $xferForm, initialParams) {
  ...
  if (data) {
    if (data.hostedError) {
      ...
      var deluxeSmtBtnTxt =
        data.deluxehpf.cmd == "payment" ? "Submit Payment" : "Save Card On File";
      ...
      if (data.deluxehpf.cmd == "payment" && data.deluxehpf.payAmount > 0) {
        $deluxeDialog.html(
          '<div class="row justify-content-center mt-3" id="deluxePayInfo"><div class="col" style="text-align: center;"><label class="mx-1">Payment Amount</label><input class="mx-1" size="10" disabled="" value="$ ' +
            data.deluxehpf.payAmount.toFixed(2) +
            '" id="deluxePayAmount" type="text"></div></div>',
        );
      }

      $deluxeDialog.append(
        '<div class="row justify-content-center mt-3"><div class="col" style="text-align: center;"><label class="mx-1">Cardholder Name</label><input class="mx-1" id="deluxeCardHolderName" type="text" size="30"></div></div>',
      );
          ...
          HostedForm.init(options, {
            onSuccess: (hpfData) => {
              ...
              $deluxeDialog
                .empty()
                .append(
                  '<div id="hhk-loading-spinner" style="width: 100%; height: 100%; margin-top: 100px; text-align: center"><img src="../images/ui-anim_basic_16x16.gif"><p>Working...</p></div>',
                );
...
export function setupCOF($chgExpand, idx) {
  ...
  if ($chgExpand.length > 0) {
    $(document)
      .find("input[name=rbUseCard" + idx + "]")
      .on("change", function () {
        if (
          ($(this).val() == 0 && $(this).prop("checked") === true) ||
          ($(this).prop("checked") === true && $(this).prop("type") === "checkbox")
        ) {
          $chgExpand.show("fade");
        } else {
          $chgExpand.hide("fade");
    ...
    if (
      $(document)
        .find("input[name=rbUseCard" + idx + "]:checked")
        .val() > 0 ||
      ($(document)
        .find("input[name=rbUseCard" + idx + "]")
        .prop("checked") === false &&
    ...
    if ($("#btnvrKeyNumber" + idx).length > 0) {
      $("#btnvrKeyNumber" + idx).change(function () {
        if (
          ($("input[name=rbUseCard" + idx + "]:checked").val() == 0 &&
            $("#btnvrKeyNumber" + idx).prop("checked") == true) ||
          ($("input[name=rbUseCard" + idx + "]").prop("checked") === true &&
            $("input[name=rbUseCard" + idx + "]").prop("type") === "checkbox")
        ) {
    ...
    if ($("#txtvdNewCardName" + idx).length > 0) {
      $("#txtvdNewCardName" + idx).keydown(function (e) {
        ...
        if ((key >= 48 && key <= 57) || (key >= 96 && key <= 105)) {
          // both number keys and numpad number keys.
          $("#lhnameerror").show();
          return false;
        }
      ...
      if ((data.success && data.success != "") || (data.COFmsg && data.COFmsg != "")) {
        flagAlertMessage(
          (data.success === undefined ? "" : data.success) +
            (data.COFmsg === undefined ? "" : data.COFmsg),
          "success",
        );
      }
...
export function paymentsTable(tableID, containerID, refreshPayments) {
  ...
  $("#" + containerID).on("click", ".hhk-voidPmt", function () {
    ...
    if (
      btn.val() !== "Saving..." &&
      confirm("Void/Reverse this payment for $" + amt.toFixed(2).toString() + "?")
    ) {
      btn.val("Saving...");
  ...
  $("#" + containerID).on("click", ".hhk-returnPmt", function () {
    ...
    if (
      btn.val() !== "Saving..." &&
      confirm("Return this payment for $" + amt.toFixed(2).toString() + "?")
    ) {
      btn.val("Saving...").addClass("hhk-loading-btn");
  ...
  $("#" + containerID).on("click", ".hhk-undoReturnPmt", function () {
    ...
    if (
      btn.val() !== "Saving..." &&
      confirm("Undo this Return/Refund for $" + amt.toFixed(2).toString() + "?")
    ) {
      btn.val("Saving...").addClass("hhk-loading-btn");

...
export function setupRegForm(
  ...
  $(".btnSave")
    .click(function () {
      ...
      var isSigned =
        $(this).closest(".ui-tabs-panel").find("div.PrintArea .signDate:visible").length > 0;
      ...
      try {
        ...
        $(this)
          ...
          .find("div.PrintArea .btnSign, div.PrintArea .btnInitial")
          .remove();
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data.idDoc > 0) {
              flagAlertMessage(
                "<strong>Success:</strong> Registration form saved successfully",
                false,
              );
              $(".btnSave").hide();
            } else {
...
export function loadSignatures(signatures) {
  ...
  try {
    for (let docId in signatures) {
      ...
      //get regforminputs
      var regFormInputs =
        docId == "vreg" ? $("#vreg .regFormInput") : $("#vsignedReg #" + docId + " .regFormInput");
      regFormInputs.attr("disabled", true);

      signatures[docId].forEach((element) => {
        if (element.type == "signature" && element.uniqId !== undefined) {
          // if is agreement signature
        ...
        } else if (element.class == "regFormInput" && element.uniqId !== undefined) {
        ...
        } else if (element.idName) {
          //original/default signature lines
          $("#" + docId + " .signWrapper[data-idname=" + element.idName + "] .sigLine img")
            .prop("src", element.signature)
            .show();
...
export function setupEsign() {
  ...
  $("#jSignDialog").dialog({
    ...
    buttons: {
      Clear: function () {
        ...
        $("#" + formCode + " .signWrapper[data-uniqid=" + uniqId + "] .sigLine img")
          .attr("src", "")
          .hide();
        $(
          "#" + formCode + " .signWrapper[data-uniqid=" + uniqId + "] .sigLine input.regFormInput",
        ).val("");
      ...
      Sign: function () {
        ...
        if (onDone() != false) {
          ...
          $(
            "#" +
              formCode +
              " .signWrapper[data-uniqid=" +
              uniqId +
              "] .sigLine input.regFormInput",
          ).val(signature);
          $("#" + formCode + " .signWrapper[data-uniqid=" + uniqId + "] .signDate").show();
          $(this).dialog("close");
        } else {
    ...
    .on("click", function () {
      ...
      if (eSignMethod === "jSign") {
        ...
        if ($(this).hasClass("btnInitial")) {
          ...
          $("#topazDialog")
            .dialog("option", "title", "Initial")
            .dialog("option", "width", getDialogWidth(225))
            .dialog("open");
        } else {
          $("#topazDialog #sigImg").attr("width", "500");
          $("#topazDialog")
            .dialog("option", "title", "Signature: " + name)
            .dialog("option", "width", getDialogWidth(550))
            .dialog("open");
        }
      }
    });
  ...
  function showAlert(msg, showInstallBtn = false, hideSigBox = true) {
    ...
    let installSigWebBtn = $("<a></a>")
      .prop("href", "https://www.topazsystems.com/software/sigweb.exe")
      .text("Install SigWeb")
      .button();
  ...
  function initTopaz() {
    ...
    if (IsSigWebInstalled()) {
      ...
      if (sigWebVer !== "") {
        ...
        if (SigWeb_1_7_0_0_IsInstalled) {
          ...
          if (SigWeb_1_6_4_0_IsInstalled) {
            ...
            showAlert(
              "A newer version of SigWeb is available. Please install the latest version: ",
              true,
            );
          }
        }
      ...
      showAlert(
        "Unable to communicate with SigWeb, please make sure it is installed and running ",
        true,
      );
    }
  }
  ...
  function onSign() {
    if (IsSigWebInstalled()) {
      ...
      if (GetTabletState() == 0) {
        showAlert(
          "Unable to connect to Signature Pad, please check that it is connected to this computer",
          false,
          true,
        );
      }

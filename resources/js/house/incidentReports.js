...
(function ($) {
  $.fn.incidentViewer = function (options) {
    var incidentdialog = $(
      '<div id="incidentDialog" class="hhk-tdbox hhk-visitdialog" title="Incident">' +
        "<form>" +
  ...
  function createActions(reportId, row) {
    ...
    // Edit icon
    $li = $('<li title="Edit Incident" data-incidentid="' + reportId + '" />').addClass(
      "hhk-report-button incident-edit ui-corner-all ui-state-default",
    );
    ...
    if (row.canDelete) {
      // Delete Edit Icon
      $li = $('<li title="Delete report" data-reportid="' + reportId + '" />').addClass(
        "hhk-report-button incident-delete ui-corner-all ui-state-default",
      );
    ...
    $li = $('<li title="Undo Delete" data-reportid="' + reportId + '" />')
      .addClass("hhk-report-button incident-undodelete ui-corner-all ui-state-default")
      .hide();
  ...
  function saveIncident($wrapper, settings, $table, print) {
    ...
    if (error == true) {
      flagAlertMessage("Incident not saved. Check fields in red.", "error");
    } else {
          ...
          if (data.idReport > 0) {
            if (print) {
              ...
              if (data.status == "success") {
                flagAlertMessage("Incident Saved successfully.", "success");
              }
  ...
  function actions($wrapper, settings, $table) {
        ...
        success: function (data) {
          if (data.title) {
            if (data.userCanEdit === false) {
              $wrapper.incidentdialog
                .find("input[name=incidentTitle]")
                .attr("readonly", "readonly");
              $wrapper.incidentdialog
                .find("input[name=incidentDate]")
                .attr("readonly", "readonly")
                .datepicker("destroy");
              $wrapper.incidentdialog
                .find("textarea[name=incidentDescription]")
                .attr("readonly", "readonly");
            }
    ...
    $wrapper.incidentdialog.on("click", ".hhk-finish-signature-btn", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.title) {
          ...
          if (data.signature) {
            body += "<td>" + '<img src="' + data.signature + '">' + "</td>";
          } else {
            body += "<td></td>";
          }
          body +=
  ...
  function createViewer($wrapper, settings) {
    if (settings.guestId > 0 || settings.psgId > 0 || settings.rid > 0) {
      $(
        '<div class="ui-button ui-corner-all ui-state-default" id="incident-create"><span class="ui-icon ui-icon-plus"></span>New Incident</div>',
      ).appendTo($wrapper);
      ...
      var dtTable = $table
        .on("draw.dt", function (e, settings) {
          ...
          if (result.length > 0) {
            ...
            $("#incidentCounts").text(
              " - " + active + " Active | " + onHold + " On Hold | " + resolved + " Resolved",
            );
          } else {

...
function chgRoomCleanStatus(idRoom, statusCode) {
  ...
  if (confirm("Change the room status?")) {
    $.post(
      "ws_resc.php",
      { cmd: "saveRmCleanCode", idr: idRoom, stat: statusCode },
      function (data) {
        if (data) {
          try {
            data = JSON.parse(data);
    ...
    function (data) {
      if (data) {
        ...
        if (data.error) {
          ...
          diag.append(
            $('<div class="hhk-panel hhk-tdbox hhk-visitdialog" style="font-size:0.8em;"/>').append(
              $(data.markup),
            ),
          );
...
function saveStatusEvent(idResc, type) {
  ...
  $.post(
    "ws_resc.php",
    $("#statForm").serialize() + "&cmd=saveStatEvent" + "&id=" + idResc + "&tp=" + type,
    function (data) {
      if (data) {
        try {
          data = JSON.parse(data);
  ...
  $.post(
    ...
    function (data) {
      ...
      if (data) {
        ...
        $diagbox.append(
          $('<div class="hhk-tdbox hhk-visitdialog" style="font-size:0.8em;"/>').append(
            $(data.success),
          ),
        );
        ...
        $replaceRoom.change(function () {
          if ($(this).val() == "new" && $changeDate.val() !== "") {
            getVisitRoomList(
              ...
              $selResource,
              $expectedDepDate.datepicker("getDate"),
            );
        ...
        $selResource.change(function () {
          ...
          if (
            ...
            (data.curResc.defaultRateCat != "" &&
              rooms[selResource].defaultRateCat != "" &&
              data.curResc.defaultRateCat != rooms[selResource].defaultRateCat)
          ) {
          ...
          "Change Rooms": function () {
            if ($("#selResource").val() > 0) {
              ...
              changeRooms(
                ...
                span,
                $selResource.val(),
                $('input[name="rbReplaceRoom"]:checked').val(),
                $cbUseDefaultRate.prop("checked"),
                $changeDate.datepicker("getDate").toUTCString(),
                expectedDeparture,
              );
              $(this).dialog("close");
            } else {
  ...
  return [
    {
      ...
      customize: function (win) {
        ...
        if (className.length > 0) {
          $(win.document.body).addClass(className);
        }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  //notes
  let notesDialog = $(
    '<div id="registerNotes" class="visitDialog" style="font-size: 0.8em;"><div class="regNotesWrapper hhk-tdbox"></div></div>',
  );
  ...
  if (showCreatedDate) {
    wlCols.push({
      ...
      title: "Created On",
      render: function (data, type) {
        return dateRender(data, type, "MMM D, YYYY H:mm");
      },
    });
  ...
  calendar = new Calendar(calendarEl, {
    ...
    headerToolbar: {
      ...
      center: "",
      right:
        "timeline1weeks,timeline2weeks,timeline3weeks,timeline4weeks refresh,today prevMonth,prev,next,nextMonth",
    },
    ...
    resourceLabelDidMount: function (info) {
      ...
      if (info.resource.extendedProps.idResc > 0) {
        ...
        info.el.onclick = function () {
          getStatusEvent(info.resource.extendedProps.idResc, "resc", info.resource.title);
        };
      }
    },
    ...
    eventDrop: function (info) {
      ...
      if (event.extendedProps.idVisit > 0 && info.delta.days !== 0) {
        if (confirm("Move Visit to a new start date?")) {
          moveVisit(
            "visitMove",
            event.extendedProps.idVisit,
            event.extendedProps.Span,
            info.delta.days,
            info.delta.days,
            true,
          );
      ...
      if (event.extendedProps.idReservation > 0) {
        ...
        if (movedDates && changedRoom) {
          ...
          if (confirm(moveMssg)) {
            moveVisit(
              "reservMove",
              event.extendedProps.idReservation,
              0,
              info.delta.days,
              info.delta.days,
              true,
    ...
    eventResize: function (info) {
      ...
      if (info.event.extendedProps.idVisit > 0) {
        if (confirm("Move check out date?")) {
          moveVisit(
            "visitMove",
            info.event.extendedProps.idVisit,
            info.event.extendedProps.Span,
            0,
            info.endDelta.days,
            true,
          );
      ...
      if (info.event.extendedProps.idReservation > 0) {
        if (confirm("Move expected end date?")) {
          moveVisit(
            "reservMove",
            info.event.extendedProps.idReservation,
            0,
            0,
            info.endDelta.days,
            true,
    ...
    eventClick: function (info) {
      ...
      if (info.event.extendedProps.idVisit && info.event.extendedProps.idVisit > 0) {
        ...
        viewVisit(
          ...
          info.event.extendedProps.idVisit,
          buttons,
          "Edit Visit #" + info.event.extendedProps.idVisit + "-" + info.event.extendedProps.Span,
          "",
          info.event.extendedProps.Span,
        );
      }
    ...
    eventDidMount: function (info) {
      if (
        hindx === undefined ||
        hindx === 0 ||
        info.event.extendedProps.idHosp === undefined ||
        info.event.extendedProps.idAssoc == hindx ||
        info.event.extendedProps.idHosp == hindx
      ) {
        ...
        if (
          !info.textColor ||
          (typeof info.textColor === "string" &&
            (info.textColor.toLowerCase() == "white" || info.textColor.toLowerCase() == "#ffffff"))
        ) {
        ...
        if (info.event.extendedProps.idReservation !== undefined) {
          info.el.title =
            ...
            (info.event.extendedProps.idResc > 0 ? ", " + resource.title : "") +
            ", " +
            info.event.extendedProps.resvStatus +
            (shoHospitalName ? ", " + info.event.extendedProps.hospName : "");
          ...
          if (info.event.extendedProps.backBorderColor != "") {
            info.el.style.cssText +=
              "box-shadow: " +
              info.event.extendedProps.backBorderColor +
              " 0px 9px 0 0; margin-bottom:10px;";
          }
          ...
          if (info.event.extendedProps.vStatusCode == "a") {
            info.el.title =
              ...
              resource.title +
              ", Status: " +
              info.event.extendedProps.visitStatus +
              ", " +
              info.event.extendedProps.guests +
              (info.event.extendedProps.guests > 1
                ? " " + visitorLabel + "s"
                : " " + visitorLabel) +
              (shoHospitalName ? ", " + hospTitle + ": " + info.event.extendedProps.hospName : "");
          } else {
            info.el.title =
              info.event.extendedProps.fullName +
              ", Room: " +
          ...
          if (
            info.event.extendedProps.extended !== undefined &&
            info.event.extendedProps.extended
          ) {
        ...
        if (data.error) {
          ...
          $("#rptInvdiv").on("click", ".invSetBill", function (event) {
            ...
            invSetBill(
              ...
              $(this).data("payor"),
              "div#setBillDate",
              "#trBillDate" + $(this).data("inb"),
              $("#trBillDate" + $(this).data("inb")).text(),
              $("#divInvNotes" + $(this).data("inb")).text(),
              "#divInvNotes" + $(this).data("inb"),
            );
          });
            ...
            if ($(this).data("stat") == "del") {
              if (
                !confirm(
                  "Delete Invoice " +
                    $(this).data("inb") +
                    ($(this).data("payor") != "" ? " for " + $(this).data("payor") : "") +
                    "?",
                )
              ) {
                return;
  ...
  $(
    "#calendar .fc-header-toolbar .fc-toolbar-chunk:nth-child(3) .fc-button-group:nth-child(1)",
  ).addClass("hideMobile");
      ...
      success: function (data) {
        if (data.totals) {
          $("#vreferrals").referralViewer({
            statuses: data.totals,
            labels: {
              patient: patientLabel,
              referralFormTitle: referralFormTitleLabel,
  ...
  $("#vstays, #vresvs, #vuncon, #vdaily, #vwls").each(function () {
    $(this)
      .find("h3 span:first")
      .after($(this).find(".dt-buttons button").css("font-size", "0.9em").addClass("ms-5"));
  });
});

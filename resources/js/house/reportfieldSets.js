...
(function ($) {
  ...
  function actions($wrapper, settings) {
    $wrapper.on("change", "#filterSets select", function () {
      var $this = $(this);
      $wrapper
        .find("#filterSetBtns button, #filterSetBtns #fieldSetName, #divFieldsetError")
        .hide();
      if ($this.val() != "") {
    ...
    $wrapper.on("change", "#fields select", function (e) {
      if (e.originalEvent) {
        ...
        if (filterSet != "") {
          ...
          $wrapper
            .find(
              "#filterSetBtns button#saveNewSet, #filterSetBtns button#saveGlobalSet, #filterSetBtns #fieldSetName",
            )
            .show();
        }
      }
        ...
        case "saveNewSet":
          ...
          success = function (data) {
            if (data.error) {
              ...
              if (data.fieldSet) {
                if (
                  $wrapper.find("#fieldset optgroup[label='" + data.fieldSet.optGroup + "']")
                    .length == 0
                ) {
                  $wrapper
                    .find("#fieldset")
                    .append("<optgroup label='" + data.fieldSet.optGroup + "'></optgroup>");
                }
                $wrapper
                  .find("#fieldset optgroup[label='" + data.fieldSet.optGroup + "']")
                  .append(
                    '<option value="' +
                      data.fieldSet.idFieldSet +
                      '">' +
                      data.fieldSet.title +
                      "</option>",
                  );
                $wrapper
                  .find("#fieldset")
                  .attr("size", function (i, v) {
                    return v * 1 + 1;
                  })
                  .val(data.fieldSet.idFieldSet)
                  .trigger("change");
              }
            }
          };
        ...
        case "saveSet":
          ...
          success = function (data) {
            if (data.error) {
              ...
              $wrapper
                .find("#fieldset option[value=" + data.fieldSet.idFieldSet + "]")
                .text(data.fieldSet.title);

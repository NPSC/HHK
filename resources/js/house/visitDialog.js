...
export function viewVisit(idGuest, idVisit, buttons, title, action, visitSpan, ckoutDates) {
  ...
  $.post(
    ...
    function (data) {
      ...
      $diagbox.append(
        $(
          '<div class="hhk-panel hhk-tdbox hhk-visitdialog" style="font-size:0.8em; width:100%"/>',
        ).append($(data.success)),
      );
        ...
        $("#tblStays").on("change", "input.hhk-ckoutCB", function () {
          ...
          if (ckout === true) {
            ...
            if (todayStr !== coDateStr && action !== "ref") {
              ...
              $diagbox.append(
                $('<div class="hhk-panel hhk-tdbox hhk-visitdialog"/>').append(
                  $('<div class="ui-autocomplete-loading" style="width:5em;">Loading</div>'),
                ),
              );
      ...
      $diagbox.dialog("option", "width", $(window).width() * 0.95);
...
export function saveFees(idGuest, idVisit, visitSpan, rtnTbl, postbackPage) {
  ...
  // show working icon
  $keysfees.append(
    '<div id="hhk-loading-spinner" style="width: 100%; height: 100%; padding-top: 100px; text-align: center"><img src="../images/ui-anim_basic_16x16.gif"><p>Working...</p></div>',
  );

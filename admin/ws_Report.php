...
function roomRevReport(PDO $dbh, $rbsel, $yr, $fyMonthsAdjust) {
    ...
    if ($rbsel == "cy") {
        ...
        if ($yr != "" && $yr != 'all') {
            $whClause = " and year(p.Payment_Date)= :yr ";
            $pArray = array(':yr' => $yr);
            $headerYears = "$yr";
        }

        $query = "select year(p.Payment_Date) as fy, ifnull(r.Title, 'Sub Total'), ";
    } else {

        if ($yr != "" && $yr != 'all') {
            $whClause = " and year(DATE_ADD(p.Payment_Date, INTERVAL $fyMonthsAdjust MONTH))= :yr ";
            $pArray = array(':yr' => $yr);
            $headerYears = "$yr";
        }
        $query = "select year(DATE_ADD(p.Payment_Date, INTERVAL $fyMonthsAdjust MONTH)) as fy, ifnull(r.Title, 'Sub Total'), ";
    }

    $query .= " sum(p.Amount - p.Balance) as `Amount`
from payment p left join payment_invoice pi on p.idPayment = pi.Payment_Id
	...
	left join resource r on v.idResource = r.idResource
            where p.Status_Code = '".PaymentStatusCode::Paid."' $whClause group by fy, r.Title with rollup;";

...
Order by `t`.`List_Order`;");
    ...
    public static function checkLookups(\PDO $dbh, $post, $labels) {
        ...
        if ($cmd == 'save') {
            ...
            if ($type === GlTypeCodes::m) {
                ...
                self::replaceLookups($dbh, $postLookups['selmisc'], $codeArray, (isset($postLookups['cbDiagDel']) ? $postLookups['cbDiagDel'] : array()));
            } else {
                self::replaceGenLk($dbh, $tableName, $codeArray, $amounts, $orderNums, (isset($postLookups['cbDiagDel']) ? $postLookups['cbDiagDel'] : NULL), $rep, (isset($postLookups['cbDiagDel']) ? $postLookups['selDiagDel'] : array()));
            }
        }
    ...
    protected static function replaceLookups(\PDO $dbh, $category, array $title, array $use)
    {
        $uS = Session::getInstance();
        $rowsAffected = 0;
     ...
     * @return float|int
     */
    public static function replaceGenLk(\PDO $dbh, $tblName, $desc, $subt, $order, $del, $replace, $replaceWith)
    {
        $rowsAffected = 0;

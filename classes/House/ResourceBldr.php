...
class ResourceBldr
{
        ...
        if (isset($desc)) {
            ...
            foreach ($desc as $k => $r) {
                ...
                if ($glRs->Substitute->getStoredVal() != $newSubt) {
                    ...
                    $defaultCode = Common::incCounter($dbh, 'codes');
    ...
    public static function getSelections(\PDO $dbh, $tableName, $type, $labels, array $descriptions = []) {
        ...
        if ($tableName == RESERV_STATUS_TABLE_NAME) {
            ...
            $lookups = Common::readLookups($dbh, $type, 'Code', true);
...
ORDER BY `t`.`List_Order`;");
              ...
              . ($tableName == DIAGNOSIS_TABLE_NAME ? HTMLTable::makeTh('Category') : '')
               . ($type == GlTypeCodes::CA ? HTMLTable::makeTh('Amount') : '')
                . ($type == GlTypeCodes::HA ? HTMLTable::makeTh('Days') : '')
                 ...
                 . ($type == GlTypeCodes::Demographics ? HTMLTable::makeTh('Icon <a href="https://icons.getbootstrap.com/" target="_blank" class="ms-2" title="More Info"><i class="bi bi-info-circle-fill"></i></a>') : '')
    ...
    public static function checkLookups(\PDO $dbh, $post, $labels) {
        ...
        if ($cmd == 'save') {
            ...
            if ($type === GlTypeCodes::m) {
                ...
                static::replaceLookups($dbh, $postLookups['selmisc'], $codeArray, (isset($postLookups['cbDiagDel']) ? $postLookups['cbDiagDel'] : array()));
            } else {
                static::replaceGenLk($dbh, $tableName, $codeArray, $amounts, $attributes, $orderNums, (isset($postLookups['cbDiagDel']) ? $postLookups['cbDiagDel'] : NULL), $rep, (isset($postLookups['cbDiagDel']) ? $postLookups['selDiagDel'] : array()));
            }
        }
     ...
     * @return float|int
     */
    public static function replaceGenLk(\PDO $dbh, $tblName, $desc, $subt, $attributes, $order, $del, $replace, $replaceWith)
    {
        $rowsAffected = 0;

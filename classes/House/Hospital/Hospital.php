...
class Hospital
{
    ...
    public static function createReferralMarkup(\PDO $dbh, HospitalStay $hstay, $offerBlankHosp = TRUE, array $referralHospitalData = [])
    {
        ...
        if ($uS->ReferralAgent) {
            ...
            if (isset($referralHospitalData['referralAgent']) && $referralHospitalData['referralAgent']['lastName'] != '') {
                ...
                if ($hstay->getAgentId() > 0) {
                    ...
                    if (
                        $referralHospitalData['referralAgent']['lastName'] != $agent->getRoleMember()->get_lastName()
                    ...
                    if (count($results) == 2 && $results[0]['id'] > 0) {
                        ...
                        $guestSubmittedAgent = HTMLContainer::generateMarkup(
                            'span',
                            'Existing Agent submitted: ' . $referralHospitalData['referralAgent']['firstName'] . ' ' . $referralHospitalData['referralAgent']['lastName']
                            . (isset($referralHospitalData['referralAgent']['phone']) ? ', ' . $referralHospitalData['referralAgent']['phone'] : '') . (isset($referralHospitalData['referralAgent']['email']) ? ', ' . $referralHospitalData['referralAgent']['email'] : ''),
                        ...
                        $wPhone = (isset($referralHospitalData['referralAgent']['phone']) ? preg_replace('~.*(\d{3})[^\d]*(\d{3})[^\d]*(\d{4}).*~', '($1) $2-$3', $referralHospitalData['referralAgent']['phone']) : '');
                        $cPhone = '';
                        $email = (isset($referralHospitalData['referralAgent']['email']) ? $referralHospitalData['referralAgent']['email'] : '');
                        $name = array('first' => $referralHospitalData['referralAgent']['firstName'], 'last' => $referralHospitalData['referralAgent']['lastName']);
                        $idName = 0;

                        $guestSubmittedAgent = HTMLContainer::generateMarkup(
                            'span',
                            'New Agent submitted: ' . $referralHospitalData['referralAgent']['firstName'] . ' ' . $referralHospitalData['referralAgent']['lastName']
                            . (isset($referralHospitalData['referralAgent']['phone']) ? ', ' . $referralHospitalData['referralAgent']['phone'] : '') . (isset($referralHospitalData['referralAgent']['email']) ? ', ' . $referralHospitalData['referralAgent']['email'] : ''),
                            array('class' => 'ui-state-highlight')
                        );
                    }
            ...
            $ratbl->addBodyTr(
                HTMLTable::makeTh(
                    HTMLContainer::generateMarkup('span', $labels->getString('hospital', 'referralAgent', 'Referral Agent')) .
        ...
        if ($uS->Doctor) {
            ...
            if (isset($referralHospitalData['doctor']) && trim($referralHospitalData['doctor']) != '') {
                ...
                if ($hstay->getDoctorId() > 0) {
                    ...
                    if ($docLast != $doc->getRoleMember()->get_lastName()) {
                        ...
                        // No, The guest's agent name is different than the one saved.
                        $guestSubmittedDoc = HTMLTable::makeTd(HTMLContainer::generateMarkup(
                            'span',
                            'Different Doctor submitted: ' . $referralHospitalData['doctor'],
                            array('class' => 'ui-state-highlight')
                        ), array('colspan' => '3'));
                    }
                    ...
                    if (count($results) == 2 && $results[0]['id'] > 0) {
                        ...
                        $guestSubmittedDoc = HTMLTable::makeTd(HTMLContainer::generateMarkup(
                            'span',
                            'Existing Doctor submitted: ' . $referralHospitalData['doctor'],
                            array('class' => 'ui-state-highlight')
                        ), array('colspan' => '3'));
                        ...
                        $guestSubmittedDoc = HTMLTable::makeTd(HTMLContainer::generateMarkup(
                            'span',
                            'New Doctor submitted: ' . $referralHospitalData['doctor'],
                            array('class' => 'ui-state-highlight')
                        ), array('colspan' => '3'));
                    }
                }
            ...
            $dtbl->addBodyTr(
                HTMLTable::makeTh(
                    HTMLContainer::generateMarkup('span', 'Doctor')
                    ...
                    . HTMLContainer::generateMarkup(
                        'span',
                        HTMLInput::generateMarkup('', array('id' => 'txtDocSch', 'class' => 'ignrSave', 'size' => '16', 'title' => 'Type 3 characters to start the search.')),
                        array('title' => 'Search', 'style' => 'margin-left:0.3em;')
                    )
                    ,
                    array('colspan' => '3')
                )
            ...
            $dtbl->addBodyTr(
                ...
                HTMLTable::makeTd(
                    ($doc->getRoleMember()->get_lastName() == '' ? '' : 'Dr. ') .
                    HTMLInput::generateMarkup(
                        $docFirst,
            ...
            if ($uS->UseDiagSearch) {
                $diagtbl->addBodyTr(
                    HTMLTable::makeTh(
                        ...
                        . HTMLInput::generateMarkup($diagId, array("type" => "text", "name" => "selDiagnosis", 'data-related' => '#diagSearch', 'class' => 'd-none hospital-stay', ...($uS->InsistResvDiag ? ['required' => 'required'] : []))),
                        array("colspan" => "2")
                    )
                );
        ...
        // Hospital stay log
        if ($hstay->getIdPsg() > 0) {
            $hstayLog = HTMLContainer::generateMarkup(
                'div',
                HTMLContainer::generateMarkup(
                    'div',
...
$(document).ready(function () {
    ...
    $(".hhk-showhsLog").click(function () {
        ...
        $div = HTMLContainer::generateMarkup(
            'div',

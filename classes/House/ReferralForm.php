...
class ReferralForm
{
	...
	public function searchGuests(\PDO $dbh, $patient, $maxGuests = self::MAX_GUESTS)
	{
		...
		if (isset($this->formUserData['guests']) && is_array($this->formUserData['guests'])) {
			...
			for ($indx = 0; $indx < $maxGuests; $indx++) {
				...
				if (
					isset($this->formUserData['guests'][$gindx]['firstName']) && isset($this->formUserData['guests'][$gindx]['lastName'])
	...
	public function datesMarkup()
	{
		...
		if (is_null($this->CkoutDT) === FALSE) {
			...
			$ckoutDate = HTMLContainer::generateMarkup(
				'span',
				'Expected Departure: '

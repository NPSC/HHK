...
class NewOccupancyReport extends AbstractReport implements ReportInterface {
...
ifnull((select SUM(DATEDIFF(least(ifnull(v.Span_End, date("' . $end . '")), date("' . $end . '")), greatest(v.Span_Start, date("' . $start . '")))) from visit v join resource re on v.idResource = re.idResource where re.Type = "rmtroom" and date(v.Span_Start) < date("' . $end . '") and date(ifnull(v.Span_End, curdate())) > date("' . $start . '")), "0") as "' . $this->rmtroomTitle . '-nights occupied",
...
';
        ...
        $helptexts["Room-nights occupied"] = "Number of nights each room was occupied, including " . $this->rmtroomTitle;
        $helptexts["Occupancy Rate"] = "Room-nights occupied / Room-nights available";
        $helptexts[$this->rmtroomTitle . "-nights occupied"] = "Number of nights each " . $this->rmtroomTitle . " was occupied";

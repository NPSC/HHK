...
(select count(distinct ng.idPsg) from stays s join name_guest ng on s.idName = ng.idName where date(s.Span_Start_Date) < date(:endDate6) and date(ifnull(s.Span_End_Date, curdate())) > date(:startDate6) and date(s.Span_Start_Date) != date(s.Span_End_Date)) as "Unique PSGs",
(select count(distinct reg.idPsg) from visit v join registration reg on v.idRegistration = reg.idRegistration where idVisit in (select fv.idVisit from vlist_first_visit fv where date(ifnull(fv.Span_End, curdate())) > date(:startDate14) and date(ifnull(fv.Span_Start, curdate())) < date(:endDate14) and date(fv.Span_Start) != date(fv.Span_End))) as "New PSGs",
(select count(distinct v.idVisit) from visit v where date(v.Span_Start) < date(:endDate8) and date(ifnull(v.Span_End, curdate())) > date(:startDate8) and date(v.Span_Start) != date(v.Span_End)) as "Total Visits",
(select ROUND(AVG(DATEDIFF(ifnull(v.Span_End, curdate()), v.Span_Start)),1) from visit v where date(v.Span_Start) < date(:endDate9) and date(ifnull(v.Span_End, curdate())) > date(:startDate9) and date(v.Span_Start) != date(v.Span_End) and v.Status not in ("p","c")) as "Average Visit Length",
(select ROUND(MEDIAN(DATEDIFF(ifnull(v.Span_End, curdate()), v.Span_Start)) over (),1) from visit v where date(v.Span_Start) < date(:endDate10) and date(ifnull(v.Span_End, curdate())) > date(:startDate10) and date(v.Span_Start) != date(v.Span_End) limit 1) as "Median Visit Length",
(select round(AVG(DATEDIFF(ifnull(v.Span_End, curdate()), v.Span_Start))) from visit v where idVisit in (select fv.idVisit from vlist_first_visit fv where date(ifnull(fv.Span_End, curdate())) > date(:startDate11) and date(fv.Span_Start) < date(:endDate11) and date(fv.Span_Start) != date(fv.Span_End))) as "Average First Visit Length",
(select round(MEDIAN(DATEDIFF(ifnull(v.Span_End, curdate()), v.Span_Start)) over (),1) from visit v where idVisit in (select fv.idVisit from vlist_first_visit fv where date(ifnull(fv.Span_End, curdate())) > date(:startDate12) and date(fv.Span_Start) < date(:endDate12) and date(fv.Span_Start) != date(fv.Span_End)) limit 1) as "Median First Visit Length";
';
        $stmt = $this->dbh->prepare($query);
        $stmt->execute([
...
group by `child/adult`;';
    ...
    public function getDiagnosisCategoryTotals(){
        ...
        $query = 'select if(d.Code is not null, ifnull(dc.Description, "Other"), "Unknown") as "Category", sum(DATEDIFF(least(ifnull(v.Span_End, date("' . $this->filter->getQueryEnd() . '")), date("' . $this->filter->getQueryEnd() . '")), greatest(v.Span_Start, date("' . $this->filter->getReportStart() . '")))) as "count"
from visit v

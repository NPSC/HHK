...
//        $isLoggedUserLocal = UserClass::isLocalUser($dbh, $uS);
        ...
        $resetPwFlag = $canResetPw ? HTMLInput::generateMarkup('1', array('id'=>'canResetPw', 'type'=>'hidden')) : '';

        return HTMLContainer::generateMarkup('div', $tbl->generateMarkup()) . $resetPwFlag;
    ...
    public static function saveUname(\PDO $dbh, $admin, $parms, $maintFlag) {
        ...
        if (isset($parms["wuname"])) {
            ...
            if(!preg_match('/^([a-z,A-Z,0-9,@,\.]{6,35})$/', $parms["wuname"])){
                $err = "Username can only contain lowercase letters, uppercase letters, numbers, '@', or '.' and must be 6-35 characters";
                return array("error"=>$err, "fieldErrors"=>array("wuname"=>array($err)));
            }

            $wUserName = filter_var($parms["wuname"], FILTER_SANITIZE_FULL_SPECIAL_CHARS);

            if (strtolower($wUserName) == 'admin') {
                $err = "'Admin' cannot be used as a username.";
                return array("error"=>$err, "fieldErrors"=>array("wuname"=>array($err)));
            }
        }

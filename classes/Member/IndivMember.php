...
class IndivMember extends AbstractMember
{
    ...
    public function createMarkupTable()
    {
        ...
        $tr = HTMLContainer::generateMarkup(
            ...
            HTMLInput::generateMarkup(
                ($this->nameRS->idName->getStoredVal() == 0 ? '' : $this->nameRS->idName->getStoredVal())
                ,
        ...
        $tr .= HTMLContainer::generateMarkup('td', HTMLSelector::generateMarkup(
            HTMLSelector::doOptionsMkup(
                ...
                $this->nameRS->Name_Prefix->getstoredVal(),
                TRUE
            ),
        ...
        $tr .= HTMLContainer::generateMarkup('td', HTMLSelector::generateMarkup(
            HTMLSelector::doOptionsMkup(
                $uS->nameLookups[GLTableNames::NameSuffix],
                $this->nameRS->Name_Suffix->getstoredVal(),
                TRUE
            ),
            array('name' => $idPrefix . 'selSuffix')
        ));
        ...
        $tr .= HTMLContainer::generateMarkup('td', HTMLSelector::generateMarkup(
            HTMLSelector::doOptionsMkup(
                HTMLSelector::removeOptionGroups($uS->nameLookups[GLTableNames::MemberStatus]),
                $this->nameRS->Member_Status->getstoredVal(),
                FALSE
            ),
            array('name' => $idPrefix . 'selStatus')
        ));
        ...
        $tr .= HTMLContainer::generateMarkup(
            ...
            HTMLSelector::generateMarkup(
                HTMLSelector::doOptionsMkup(
                    ...
                    $this->nameRS->Member_Type->getstoredVal(),
                    FALSE
                ),
                array('name' => $idPrefix . 'selMbrType')
            )
        );
    ...
    public function createMiscTabsMarkup(\PDO $dbh)
    {
        ...
        $tabs .= HTMLContainer::generateMarkup(
            'li',
    ...
    public function createDemographicsPanel(\PDO $dbh, $limited = FALSE, $includeBirthDate = TRUE, $demographicsUserData = [], $includeInsurance = false)
    {
        ...
        $tbl2->addBodyTr(
            HTMLTable::makeTd(
                'Deceased: ' . HTMLInput::generateMarkup('', $deAttr)
                . HTMLInput::generateMarkup('0', array('type' => 'hidden', 'name' => 'cbMarker_deceased'))
                ,
                array('class' => 'tdlabel', 'title' => 'Check if deceased.')
            )
            . HTMLTable::makeTd(HTMLContainer::generateMarkup(
                'div',
                'Date: ' . HTMLInput::generateMarkup(($this->get_DateDeceased() == '' ? '' : date('M j, Y', strtotime($this->get_DateDeceased()))), array('name' => $idPrefix . 'txtDeathDate', 'class' => 'ckbdate')),
                $dateAttr
            ))
        );
        ...
        $tbl2->addBodyTr(
            HTMLTable::makeTd(
                'Background Check: ' . HTMLInput::generateMarkup('', $bcAttr)
            ...
            . HTMLTable::makeTd(HTMLContainer::generateMarkup(
                'div',
                'Date: ' . HTMLInput::generateMarkup(($this->get_DateBackgroundCheck() == '' ? '' : date('M j, Y', strtotime($this->get_DateBackgroundCheck()))), array('name' => $idPrefix . 'txtBackgroundCheckDate', 'class' => 'ckbdate')),
                $bcdateAttr
            ))
        );
...
ORDER BY `List_Order`");
        ...
        foreach ($insTypes as $i) {
            ...
            $tbl->addBodyTr(
                $tbl->maketd(Labels::getString('Insurance', 'groupNumber', 'Group Number'), array('class' => "tdlabel"))
            ...
            $tbl->addBodyTr(
                $tbl->makeTd(Labels::getString('Insurance', 'memberNumber', 'Member Number'), array('class' => "tdlabel"))
...
ORDER BY `List_Order`");
    ...
    public function createOrgMarkup(array $rel, $page = "NameEdit.php")
    {
        ...
        $table->addBodyTr(
            // title
            HTMLTable::makeTd(
                HTMLInput::generateMarkup($this->get_title(), array('name' => 'txtTitle', 'size' => '10'))
            )
        );

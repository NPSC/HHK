...
class VolCal {
            ...
            if (count($rows) == 1) {
                ...
                if (($cats->get_IdName() == $evt->get_idName() && $cats->runAuthorization($evt->get_volCat(), $evt->get_volCode(), $evt->get_idName()))
                    ...
                    if ($rptId == 0 || $delAll == 0) {
                        ...
                        $query = "update mcalendar set E_Status = '" . VolCalendarStatus::Deleted . "', Last_Updated=now(), Updated_By='" . $cats->get_Username() . "' where idmcalendar = '$idMcal';";
                    } else {
                        //$query = "delete from mcalendar where idmcalendar = '$eId' or ( E_Rpt_Id > 0 and E_Rpt_Id = $rptId and Date(E_Start) > now());";
                        $query = "update mcalendar set E_Status = '" . VolCalendarStatus::Deleted . "', Last_Updated=now(), Updated_By='" . $cats->get_Username() . "' where idmcalendar = $idMcal or ( E_Rpt_Id > 0 and E_Rpt_Id = $rptId);";
                    }
    ...
    public static function UpdateCalEvent(\PDO $dbh, cEventClass $evt, UserCategories $cats) {
        ...
        if ($calRS->E_Status == VolCalendarStatus::Deleted) {
        ...
        // Log the time?
        if ($evt->get_logTime() == 1 && $calRS->E_Status->getStoredVal() != VolCalendarStatus::Logged) {
            ...
            if ($endTicks < time()) {
                ...
                $calRS->E_Status->setNewVal(VolCalendarStatus::Logged);
    ...
    public static function getEvent(\PDO $dbh, $eid) {
        ...
        $stmt->bindValue(":stat", VolCalendarStatus::Deleted, \PDO::PARAM_STR);
            ...
            foreach ($rows as $r) {
                ...
                for ($i = 0; $i < count($cols); $i++) {
                    ...
                    if ($cols[$i] == "E_Start") {
                        ...
                        if ($r[$cols[$i]] == VolCalendarStatus::Logged) {
...
//
    ...
    protected static function insertEvents(\PDO $dbh, cEventClass $firstEvent, array $repeatedEvents, $userName, $rptId) {
        ...
        foreach ($repeatedEvents as $evt) {
            ...
            $calRS->E_Status->setNewVal(VolCalendarStatus::Active);

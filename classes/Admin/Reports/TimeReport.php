...
class TimeReport
{
    public static function processTime(PDO $dbh, &$selCtrls, selCtrl &$selRptType, $fyMonthsAdjust) {
        ...
        if ($wClause != "") {
            ...
            */
            if (isset($_POST["btnMlCat"]) || isset($_POST["btnMlCatDL"])) {
                $query = "Select c2.Id from (" . self::makeSQL($wClause, $groupBy, $sumHours) . ") as c2 ";
            } else {
                $query = self::makeSQL($wClause, $groupBy, $sumHours);
            }
    ...
    private static function makeSQL($whereClause, $groupBy, $sumHours) {
        $sum = "";
        $endPar = "";
        $showStartEnd = "";

  ...
  var _source = function (
    ...
    if (term in cache) {
      ...
      if (terms.length > 1) {
        bldr =
          "\\b(" +
          $.ui.autocomplete.escapeRegex(terms[0]) +
          ").+\\b(" +
          $.ui.autocomplete.escapeRegex(terms[1]) +
          ")" +
          "|\\b(" +
          $.ui.autocomplete.escapeRegex(terms[1]) +
          ").+\\b(" +
  ...
  const _source = function (request, response, cache, shoNew, inputParms, minChars) {
    ...
    if (term in cache) {
      ...
      if (terms.length > 1) {
        ...
        bldr =
          "\\b(" + $.ui.autocomplete.escapeRegex(request.term.toString().replace(",", "")) + ")";
      }
      ...
      if (shoNew) {
        filtered.unshift({
          id: 0,
          fullName:
            '<span class="ui-icon ui-icon-plusthick"></span><span style="padding-left:1em;">New Person</span>',
        });
      ...
      if (filtered.length > maxLength) {
        filtered.unshift({
          id: "a",
          substitute: "Keep typing... (More than " + maxLength + " results found)",
        });
      }
      ...
      $.getJSON("../house/roleSearch.php", inputParms, function (data) {
        ...
        if (data.total > data.limit) {
          data.results.unshift({
            id: "a",
            substitute: "Keep typing... (showing " + data.limit + " of " + data.total + " results)",
          });
  ...
  if (txtCtrl.autocomplete("instance") !== undefined) {
    txtCtrl.autocomplete("instance")._renderItem = function (ul, item) {
      ...
      if (item.phone != "" || item.city != "" || item.state != "") {
        detailsRow =
          "<div class='autocompleteItemDetails'>" + item.phone + item.city + item.state + "</div>";
      }
      ...
      if (item.memberStatus != "" || item.noReturn != "" || item.room != "") {
        rightContent =
          "<div class='right'>" + item.memberStatus + item.noReturn + item.room + "</div>";
      }

      return $("<li>")
        .append(
          "<div style='font-size:.85em;'><div class='hhk-flex'><div class='left'>" +
            firstRow +
            detailsRow +
            mrnRow +
            "</div>" +
    ...
    txtCtrl.autocomplete("instance")._resizeMenu = function () {

...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (tabActive.length > 0) {
    let tabIndex = $('#mainTabs ul a[href="' + tabActive + '"]')
      .parent()
      .index();
    $("#mainTabs").tabs("option", "active", tabIndex);
  }
  ...
  $("#selFormUpload").change(function (e, changeEventData) {
    ...
    $.post("ResourceBuilder.php", { ldfm: $(this).val() }, function (data) {
      $("#spnFrmLoading").hide();
      ...
      if (data) {
        ...
        if (changeEventData && changeEventData.docCode) {
          $("#regTabDiv")
            .find("li.ui-tab[aria-selected=false] #docTab-" + changeEventData.docCode)
            .click();
        }
      }
    });
  ...
  $("#mainTabs").on("change", ".hhk-selLookup", function () {
    ...
    $.post("ResourceBuilder.php", { table: table, cmd: "load", tp: type }, function (data) {
      $sel.prop("disabled", false);
      if (data) {
        $sel
          .closest("form")
          .children("div.lookupDetailTbl")
          .empty()
          .append(data)
          .find(".sortable tbody")
          .sortable({
  ...
  $("#btndemoSave")
    .click(function () {
      ...
      $.post(
        "ResourceBuilder.php",
        "lookups=" +
          JSON.stringify($frm.serializeJSON()) +
          "&cmd=save" +
          "&table=" +
          "Demographics" +
          "&tp=" +
...
function bsIconAutocomplete(container) {
  container.find("input.bs-icon").each(function (i, v) {
    $(v)
      .autocomplete({
        source: bsIconList,
        ...
        select: function (event, ui) {
          if (ui.item) {
            ...
            $(this)
              .closest("td")
              .find(".demogIconPreview")
              .attr("class", "demogIconPreview mx-2 " + ui.item.value);
            return false;
          }
        },
      })
      ...
      return $("<li>")
        .append(
          '<div><i style="font-size: 1.1em" class="me-2 ' +
            item.value +
            '"></i>' +
            item.label +
            "</div>",

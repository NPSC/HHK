function invPay(id, pbp, dialg) {
  ...
  $("#keysfees")
    .empty()
    .append(
      '<div id="hhk-loading-spinner" style="width: 100%; height: 100%; padding-top: 50px; text-align: center"><img src="../images/ui-anim_basic_16x16.gif"><p>Working...</p></div>',
    );
    ...
    function (data) {
      ...
      if (data) {
        ...
        if (data.error) {
          ...
          $("#keysfees").dialog("option", "width", 900);
...
function invSetBill(inb, name, idDiag, idElement, billDate, notes, notesElement) {
  ...
  var buttons = {
    Save: function () {
      ...
      if (
        dialg.find("#txtBillDate").val() != "" &&
        dialg.find("#txtBillDate").datepicker("getDate")
      ) {
      ...
      $.post(
        "ws_resc.php",
        { cmd: "invSetBill", inb: inb, date: dt, ele: idElement, nts: nt, ntele: notesElement },
        function (data) {
          if (data) {
            try {
              data = $.parseJSON(data);
...
function invoiceAction(idInvoice, action, eid, container, show) {
  ...
  $.post(
    "ws_resc.php",
    { cmd: "invAct", iid: idInvoice, x: eid, container: container, action: action, sbt: show },
    function (data) {
      if (data) {
        try {
          data = $.parseJSON(data);

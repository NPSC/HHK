    ...
    buttons: {
      "Download to MS Word": function () {
        ...
        if ($hdnCfmRid.length > 0) {
          ...
          $confForm.append(
            $(
              '<input name="hdnCfmRid" type="hidden" value="' +
                $("#btnShowCnfrm").data("rid") +
                '"/>',
            ),
          );
        }
        if ($hdnCfmDocCode.length > 0) {
          ...
          $confForm.append(
            $(
              '<input name="hdnCfmDocCode" type="hidden" value="' +
                $('div[id="confirmTabDiv"] ul .ui-tabs-active').attr("data-docId") +
                '"/>',
            ),
          );
        }
        if ($hdnCfmAmt.length > 0) {
          ...
          $confForm.append(
            $('<input name="hdnCfmAmt" type="hidden" value="' + $("#spnAmount").text() + '"/>'),
          );
        }
        if ($hdnTabIndex.length > 0) {
          ...
          $confForm.append(
            $(
              '<input name="hdnTabIndex" type="hidden" value="' +
                $('div[id="confirmTabDiv"] ul .ui-tabs-active').attr("aria-controls") +
                '"/>',
            ),
          );
        }
      ...
      "Send Email": function () {
        ...
        $.post(
          "ws_ckin.php",
          {
            ...
            eml: "1",
            eaddr: $("#confEmail").val(),
            ccAddr: $("#ccConfEmail").val(),
            amt: $("#spnAmount").text(),
            notes: $("#tbCfmNotes" + tabIndex).val(),
            tabIndex: tabIndex,
          },
          function (data) {
  ...
  $("#btnDone").click(function () {
    ...
    if (pageManager.verifyInput() === true) {
      ...
      try {
        ...
        jsonFetch(
          jsonObject,
          "ws_resv.php", // returns text object
          (text) => {
            ...
            if (responseData.receiptMarkup && responseData.receiptMarkup != "") {
              const idPayment =
                typeof responseData.idPayment == "number" ? responseData.idPayment : false;
              const billToEmail =
                typeof responseData.billToEmail == "string" ? responseData.billToEmail : "";
              showReceipt(
                "#pmtRcpt",
                responseData.receiptMarkup,
                "Payment Receipt",
                550,
                idPayment,
                billToEmail,
            ...
            if (responseData.deleted) {
              ...
              if (responseData.warning === undefined) {
                flagAlertMessage(
                  responseData.resvTitle +
                    " Saved. " +
                    (responseData.resv.rdiv.rStatTitle === undefined
                      ? ""
                      : " Status: " + responseData.resv.rdiv.rStatTitle),
                  "success",
                );
              }
            } else {
              flagAlertMessage(
                (responseData.resvTitle === undefined ? "" : responseData.resvTitle) + " Saved. ",
                "success",
              );
            }
